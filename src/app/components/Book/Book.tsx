...
import type { BookHandles, BookPropsType } from "./type";
...
export const Book = React.forwardRef(
  (
    props: BookPropsType,
    ref:
      | ((instance: BookHandles | null) => void)
      | MutableRefObject<BookHandles | null>
      | null
  ) => {
    const {
    ...
    const { readStatus } = useViewModel({ data });
    ...
    if (readingData.isShowReadingStatus && readStatus.progress === 0)
      return null;
